...
class AntrianMcd{
    ...
    public function updateData(){
        ...
        $this->selisih_kedatangan=htmlspecialchars(strip_tags($this->selisih_kedatangan));
        $this->awal_layanan=htmlspecialchars(strip_tags($this->awal_layanan));
        $this->selisih_pelayanan=htmlspecialchars(strip_tags($this->waktu_kedatangan));
        $this->waktu_selesai=htmlspecialchars(strip_tags($this->waktu_selesai));
        $this->selisih_akhir=htmlspecialchars(strip_tags($this->selisih_akhir));
    ...
    public function generateAntrian(){
        $sqlQuery = "SELECT MIN(selisih_kedatangan) AS min_sk, MAX(selisih_kedatangan) AS max_sk,AVG (selisih_kedatangan) AS avg_sk, MIN(selisih_pelayanan) AS min_sp, MAX(selisih_pelayanan) AS
        max_sp, AVG(selisih_pelayanan) AS avg_sp FROM " . $this->db_table;
        $stmt = $this->conn->prepare($sqlQuery);
        $stmt->execute();
        $dataRow = $stmt->fetch(PDO::FETCH_ASSOC);
